...
* edited by Steven Vondruska to change stage size and to pause the video stream
*
**/
...
package {
    ...
    import Base64;
    [SWF(backgroundColor="0x000000" , width="1280" , height="720")]
    public class sAS3Cam extends Sprite {
        ...
        private function setVideoCamera(useCamera:Camera):void {
            ...
            video = new Video(useCamera.width,useCamera.height);
            video.smoothing  = doSmoothing;
            video.deblocking = doDeblocking;
            video.attachCamera(useCamera);
            addChild(video);

            debug('set width: ' + camResolution[0]);
            debug('set height: ' + camResolution[1]);
        }
        ...
        public function sAS3Cam():void {
            ...
            stage.align = this.loaderInfo.parameters["StageAlignMode"]; // empty string is absolute center
            ...
            if (null != camera) {
                if (ExternalInterface.available) {
                    ...
                    debug('width: ' + camResolution[0]);
                    debug('height: ' + camResolution[1]);

                    debug('stage width: ' + stage.stageWidth);
                    debug('stage height: ' + stage.stageHeight);
        ...
        private function debug(debugText:String):Boolean {
            var target:String = this.loaderInfo.parameters["callTarget"];
            return ExternalInterface.call(target + ".debug", debugText);
        }
